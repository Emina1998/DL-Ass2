...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) #line 1: compute queries from full resolution

        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W) #reduce spatial resolution of x and apply normalization
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1) #reduce spatial resolution of x and apply normalization
            x_ = self.norm(x_) #reduce spatial resolution of x and apply normalization
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) #computes keys and values from reduced x
        else:
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W):
        #TODO implement the forward pass of one Transformer block - done
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        ...
        patch_output_size = math.floor((img_size + 2 * (patch_size // 2) - patch_size) / stride + 1)
        self.H, self.W = patch_output_size,patch_output_size
